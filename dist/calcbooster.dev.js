...
function ten_questions() {
  for (var i = 0; i < 10; i++) {
    var operandone = Math.floor(Math.random() * 1000) + 100;
    var operator = array_for_operators[Math.floor(Math.random() * array_for_operators.length)];
    var operandtwo = Math.floor(Math.random() * 100);
    var result = Math.floor(eval(operandone + operator + operandtwo));
    correct_answers.push(result); //create element tag and to insert the operand and operators to the document

    var question_tag = document.createElement('li');
...
function get_marks() {
  ...
  user_answers = [].map.call(items, function (item) {
    return item.value;
  });
